...
<h2 class="text-center"><strong>系统全部博客</strong></h2>
<!-- 博文摘要视图-->
...
<!-- 分页功能 start -->
	<div align="center">
		<font size="2">共 ${page.getTotalPageCount()} 页</font> <font size="2">第
			${page.getPageNow()} 页</font> <a href="${pageContext.request.contextPath}/home.htm?pageNow=1">首页</a>
		<c:choose>
			<c:when test="${page.getPageNow() - 1 > 0}">
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=${page.getPageNow() - 1}">上一页</a>
			</c:when>
			<c:when test="${page.getPageNow() - 1 <= 0}">
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=1">上一页</a>
			</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${page.getTotalPageCount()==0}">
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=${page.getPageNow()}">下一页</a>
			</c:when>
			<c:when test="${page.getPageNow() + 1 < page.getTotalPageCount()}">
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=${page.getPageNow() + 1}">下一页</a>
			</c:when>
			<c:when test="${page.pageNow + 1 >= page.getTotalPageCount()}">
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=${page.getTotalPageCount()}">下一页</a>
			</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${page.getTotalPageCount()==0}">
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=${page.getPageNow()}">尾页</a>
			</c:when>
			<c:otherwise>
				<a href="${pageContext.request.contextPath}/home.htm?pageNow=${page.getTotalPageCount()}">尾页</a>
			</c:otherwise>
		</c:choose>
	</div>
	<!-- 分页功能 End -->
...
<c:if test="${ empty blogList}">
	...
	<h5 class="text-center">空空如也，暂无博客可浏览!请登录去写自己的博客吧~~~</h5>
	</c:if>